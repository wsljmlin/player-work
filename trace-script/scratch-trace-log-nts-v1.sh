...
start_trace(){
    ...
    then
        ...
        echo "id == 192 || id == 91" > /sys/kernel/tracing/events/raw_syscalls/filter
...
init_cmds_loglevels() {
    ...
    setprop debug.stagefright.c2-debug 17
    if [ "$logon" == "on" ]
    then
        if [ $kernelver == "5.15" ]
        then
    ...
    then
        ...
        lastdrop=`cat /sys/class/video_composer/drop_cnt | sed -n 's/.*player_drop_cnt: \(.*\), vpp_drop_cnt.*/\1/p'`
    fi
...
vc_detect_drop_scratch_trace() {
    ...
    dropcnt=`cat /sys/class/video_composer/drop_cnt | sed -n 's/.*player_drop_cnt: \(.*\), vpp_drop_cnt.*/\1/p'`
